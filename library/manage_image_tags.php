...
$result = mysqli_query($conn, "SELECT Tag, Image_url, Number FROM image_tags WHERE Pending = 1 ORDER BY Number ASC");
...
{
include '../../opendb_gigi.php';
$result = mysqli_query($conn, "SELECT Tag, Image_url, Number FROM image_tags WHERE Pending = 0 ORDER BY Number DESC");
while($row = mysqli_fetch_array($result)){
$Tag = $row['Tag'];
$Image_url2 = $row['Image_url'];
$Number = $row['Number'];
$result3 = mysqli_query($conn, "SELECT Url FROM images WHERE Src = '$Image_url2'");
...
$Tag = $row['Tag'];
$Image_url = $row['Image_url'];
$update = "UPDATE image_tags SET Pending = 0 WHERE Number = '$Number'";
mysqli_query($conn, $update);
$result4 = mysqli_query($conn, "SELECT Url FROM images WHERE Src = '$Image_url'");
...
include '../../opendb_gigi.php';
$delete = $_POST['Delete'];
$delete1 = "DELETE FROM image_tags WHERE Number=('$delete')";
